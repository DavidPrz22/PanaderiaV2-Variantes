...
interface ProductoFinalItem {
  unidad_venta_nombre: string;
  categoria_nombre: string;
}

export default function PFFiltersPanel() {
  const {
    showFiltersPanel,
    setShowFiltersPanel,
  ...
  const toggleCategoria = (cat: string) => {
    setSelectedCategoriasProductoFinal((prev) =>
      prev.includes(cat)
        ? prev.filter((c) => c !== cat)
        : [...prev, cat],
    );
      ...
      <div className="grid grid-cols-2 gap-6 overflow-auto pr-1 custom-scrollbar">
        <div className="flex flex-col gap-2">
          <p className="font-medium text-[13px] text-gray-700 flex items-center gap-2">
            <span className="w-1.5 h-4 rounded bg-indigo-400" /> Unidad de Venta
          </p>
